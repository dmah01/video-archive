        "use client";
            ...
            type VideoFiltersProps = {
              ...
              peopleFilterMode: "all" | "only" | "any";
              selectedGenres: number[];
              genreFilterMode: "all" | "only";
              ...
              setPeopleFilterMode: (value: "all" | "only" | "any") => void;
              setSelectedGenres: (value: number[]) => void;
              setGenreFilterMode: (value: "all" | "only") => void;
            ...
            const popupClass =
              "fixed inset-x-2 bottom-2 z-[70] max-h-[45dvh] overflow-y-auto rounded-xl pb-[env(safe-area-inset-bottom)] border border-zinc-800 bg-zinc-950 p-1.5 shadow-2xl shadow-black/50 sm:absolute sm:inset-x-auto sm:bottom-auto sm:max-h-[50vh] sm:w-[520px] sm:max-w-[calc(100vw-1rem)] sm:rounded-2xl sm:p-2.5";
            ...
            export default function VideoFilters({
              ...
              const [draftPeopleFilterMode, setDraftPeopleFilterMode] = useState<"all" | "only" | "any">(peopleFilterMode);
              const [draftGenres, setDraftGenres] = useState<number[]>(safeSelectedGenres);
              const [draftGenreFilterMode, setDraftGenreFilterMode] = useState<"all" | "only">(genreFilterMode);
              ...
              useEffect(() => {
                function handleClickOutside(event: MouseEvent) {
                  if (
                    ...
                    setDraftPeopleFilterMode(peopleFilterMode);
                    setDraftGenres(safeSelectedGenres);
                    setDraftGenreFilterMode(genreFilterMode);
              ...
              const applyMenu = () => {
                ...
                setPeopleFilterMode(draftPeopleFilterMode);
                setSelectedGenres(draftGenres);
                setGenreFilterMode(draftGenreFilterMode);
              ...
              const dateButtonText = date
                ? date.replace(/^(\d{4})-(\d{2})-(\d{2})$/, (_match, year, month, day) =>
                    `${year}.${month}.${day}.`
                  )
                : "전체";
            ...
            >
                      ...
                      <div className="relative">
                        ...
                        {openFilter === "genres" && (
                          <div className={`filter-popup ${popupClass} !max-h-none !overflow-visible sm:left-0 sm:top-[calc(100%+8px)] sm:!w-[640px] sm:!max-w-[calc(100vw-1rem)]`}>
                            <div className="mb-1.5 flex items-center justify-between sm:mb-2">
                              <span className="text-[11px] font-medium text-zinc-300 sm:text-xs">
                                장르 선택
                              </span>
                              ...
                              >
                                {([
                                  ...
                                  <button
                                    key={value}
                                    type="button"
                                    className={draftGenreFilterMode === value ? "active" : ""}
                                    onClick={() => setDraftGenreFilterMode(value)}
                                    aria-pressed={draftGenreFilterMode === value}
                                    style={{ fontSize: "12px" }}
                                  >
                                    {label}
                                  </button>
                            ...
                            <div className="flex flex-wrap items-start gap-1.5 pr-1 sm:gap-2">
                      ...
                      <div className="relative">
                        ...
                        {openFilter === "types" && (
                          <div className={`filter-popup ${popupClass} sm:left-0 sm:top-[calc(100%+8px)] sm:!w-[600px] sm:!max-w-[calc(100vw-1rem)]`}>
                            <div className="mb-1.5 flex items-center justify-between sm:mb-2">
                              <span className="text-[11px] font-medium text-zinc-300 sm:text-xs">
                                타입 선택
                              </span>
                      ...
                      <div className="relative">
                        ...
                        {openFilter === "series" && (
                          <div
                            className={`${popupClass} !max-h-none !overflow-visible !w-[680px] !max-w-[calc(100vw-1rem)] sm:right-0 sm:top-[calc(100%+8px)]`}
                          >
                      ...
                      <div className="relative">
                        ...
                        {openFilter === "date" && (
                          <div
                            className={`filter-popup ${popupClass} sm:right-0 sm:top-[calc(100%+8px)] sm:!w-[300px] sm:max-w-[calc(100vw-1rem)]`}
                          >
                            <div className="mb-1.5 flex items-center justify-between sm:mb-2">
                              <span className="text-[11px] font-medium text-zinc-300 sm:text-xs">
                                날짜 선택
                            ...
                            <div className="relative h-10 w-full">
                              <div
                                className="pointer-events-none flex h-full w-full items-center justify-between rounded-lg border border-zinc-800 bg-white pl-3 pr-1.5 text-sm text-zinc-900 outline-none transition sm:h-10 sm:rounded-lg sm:text-xs"
                                aria-hidden="true"
                              >
                                <span>{date ? dateButtonText : "년. 월. 일."}</span>
                                <svg
                                  viewBox="0 0 24 24"
                                  fill="none"
                                  stroke="currentColor"
                              ...
                              <input
                                ...
                                aria-label="날짜 선택"
                                className="absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0 px-3"
                              />
                            </div>
                            ...
                            <button
                              ...
                              className="mt-1 min-h-8 w-full rounded-lg bg-white py-1.5 text-[10px] font-semibold text-black sm:hidden"
                            >
                              적용
                            </button>
                          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        {selectedPersonData.map((person) => (
                          <button
                            ...
                            className={`inline-flex min-h-7 max-w-full shrink-0 items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] leading-4 transition hover:opacity-80 sm:min-h-7 sm:text-[11px] ${personColors[person.name] ?? "border-zinc-700 bg-zinc-900 text-zinc-300"}`}
                        ...
                        {selectedGenreData.map((genre) => (
                          <button
                            ...
                            className="inline-flex min-h-7 max-w-full shrink-0 items-center gap-1 rounded-full border border-purple-400/40 bg-purple-400/15 px-2.5 py-1 text-[10px] leading-4 text-purple-300 transition hover:opacity-80 sm:min-h-7 sm:text-[11px]"
                        ...
                        {selectedTypeData.map((type) => (
                          <button
                            ...
                            className="inline-flex min-h-7 max-w-full shrink-0 items-center gap-1 rounded-full border border-blue-400/30 bg-blue-400/10 px-2.5 py-1 text-[10px] leading-4 text-blue-300 transition hover:opacity-80 sm:min-h-7 sm:text-[11px]"
                        ...
                        {selectedSeriesData.map((item) => (
                          <button
                            ...
                            className="inline-flex min-h-7 max-w-full shrink-0 items-center gap-1 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2.5 py-1 text-[10px] leading-4 text-emerald-300 transition hover:opacity-80 sm:min-h-7 sm:text-[11px]"
                        ...
                        {date && (
                          <button
                            type="button"
                            onClick={() => setDate("")}
                            className="inline-flex min-h-7 shrink-0 items-center gap-1 rounded-full border border-zinc-700 bg-zinc-800 px-2.5 py-1 text-[10px] leading-4 text-zinc-300 transition hover:border-zinc-600 hover:bg-zinc-700 sm:min-h-7 sm:text-[11px]"
                            title="날짜 필터 해제"
                          >
